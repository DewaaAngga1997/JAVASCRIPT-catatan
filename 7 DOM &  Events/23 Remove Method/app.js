// remove();         use it to remove the selected element
// click();          use it to click on the selected element

document.getElementById("remove").addEventListener("click",()=>{
    document.getElementById("text").remove();
});

document.getElementById("goToLink").addEventListener("click", ()=>{
    document.getElementById("link").click();
});






























// ------------------------  يتم استخدامها لإعادة تعيين قيم جميع العناصر في نموذج
// reset();                                     (مثل النقر على زر إعادة الضبط)
// submit();  (يتم استخدامها لعمل سوبميت ل فورم(مثل النقر على زر من نوع سوبميت
// focus();     يتم استخدامها لإضافة تركيز إلى عناصر الذي يمكنك اضافة تركيز لها
// blur();                  يتم استخدامها للحذف التركيز من عنصر مضاف اليه تركيز 
